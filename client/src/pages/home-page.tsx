            ...
            {user && (
              <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-1">
                  {[...Array(user.remainingDesigns <= 6 ? user.remainingDesigns : 6)].map((_, i) => (
                    <div key={i} className="w-3 h-6 bg-[#39FF14] rounded"></div>
                  ))}
                  {[...Array(Math.max(0, 6 - (user.remainingDesigns <= 6 ? user.remainingDesigns : 6)))].map((_, i) => (
                    <div key={i} className="w-3 h-6 bg-gray-200 rounded"></div>
                  ))}
                </div>
                <span className="text-sm font-medium">
                  {user.subscriptionTier === 'pro' ? 'Unlimited' : `${user.remainingDesigns}/6`}
                </span>
              </div>
            )}
          </div>
